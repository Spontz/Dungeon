...
Protected Module Files
	...
	#tag Method, Flags = &h0
		Sub cleanDataFolders()
		  ...
		  file = file.Child("Engines").Child("Inferno")
	...
	#tag Method, Flags = &h0
		Sub CopyFileOrFolder(source as FolderItem, destination as FolderItem, optional destinationName as String)
		  ...
		  if source = nil then
		    RaiseError ("A needed file could not be located", "Your demo editor installation may be corrupt. Please reinstall it.")
		    exit
		  end if
		  
		  if not source.exists then
		    RaiseError ("The file '" + source.Name + "' could not be located", "Your project may be corrupt")
		    exit
		  end if
