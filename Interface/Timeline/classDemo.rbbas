...
Protected Class classDemo
	...
	#tag Method, Flags = &h0
		Sub Constructor(optional demoFile as folderitem)
		  ...
		  // Set the data folder
		  me.SetDataFolder(GetFolderItem("Engines").child("Inferno").child("data_" + engines.getHash))
	...
	#tag Method, Flags = &h0
		Sub deleteFolder(folderID as string)
		  ...
		  totalFiles = demoDB.SQLSelect("SELECT COUNT(*) as total FROMFILESWHERE parent = " + folderID).Field("total").IntegerValue
		  
		  if totalFiles > 0 then
		    ExecuteSQL("DELETE FROMFILESWHERE parent=" + folderID)
		  end if
	...
	#tag Method, Flags = &h0
		Sub deleteResource(resourceID as string)
		  ...
		  ExecuteSQL("DELETE FROMFILESWHERE id=" + resourceID)
	...
	#tag Method, Flags = &h0
		Function moveResource(data as string, parentID as string) As string
		  ...
		  '
		  '// Return the ID of the added folder
		  'result = demoDB.SQLSelect("SELECT id FROMFILESORDER BY id DESC LIMIT 1").Field("id").StringValue
		  '
		  'return result
		  '
		  'else
	...
	#tag Method, Flags = &h0
		Function saveFolder(ID as string, destination as FolderItem, optional includeContents as boolean) As folderitem
		  ...
		  if includeContents then
		    ...
		    names = demoDB.SQLSelect("SELECT name, id FROMFILESwhere parent='" + ID + "'")
	...
	#tag Method, Flags = &h0
		Sub saveProject(saveAs as boolean)
		  if projectFolder = nil or saveAs then
		    ...
		    projectFolder = GetSaveFolderItem(FileTypes.SVEditorProject, "Untitled Project.spp")
	...
	#tag Method, Flags = &h0
		Sub SetEnginesFolder(folder as folderitem)
		  ...
		  if not folder.Child("inferno").Exists then
		    folder.child("Inferno").CreateAsFolder
		  end if
