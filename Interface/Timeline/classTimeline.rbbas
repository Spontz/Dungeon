...
Inherits Canvas
		  ...
		  if key.Asc = 8 or key.Asc = 127 then
		    ...
		    for i=0 to UBound(selectedBarIDs)
		      if demo.getBarType(str(selectedBarIDs(i))) <> "" then
		        ...
		        engines.deleteBar(selectedBarIDs(i))
		      end if
		    next
		...
		Function MouseDown(X As Integer, Y As Integer) As Boolean
		  ...
		  if clickedItem = "horizontalRuler" then
		    if Keyboard.AsyncAltKey or Keyboard.AsyncOptionKey then
		      ...
		      engines.setCurrentTime(time)
		      
		    elseif time <= demo.getDemoStartTime or time <= engines.EnginesList(engines.mainEngine).runTime then
		    ...
		    elseif time >= demo.getDemoEndTime or time >= engines.EnginesList(engines.mainEngine).runTime then
		  ...
		  elseif NthField(clickedItem, " ", 1) = "BarID" and val(NthField(clickedItem, " ", 2)) > 0  then
		    ...
		    elseif  keyboard.OptionKey and demo.countSelectedbars > 0 then
		      ...
		      'for i = 0 to Ubound(NewSectionList)
		      'engines.createSection(NewSectionList(i))
		      'next
	...
	#tag Event
		Sub MouseUp(X As Integer, Y As Integer)
		  ...
		  'engines.setBarStartTime str(GetSelectedSectionID(i)), SelectedSections(i, 1)
		  'engines.setBarEndTime str(GetSelectedSectionID(i)), SelectedSections(i, 2)
		  'engines.setBarLayer str(GetSelectedSectionID(i)), SelectedSections(i, 5)
		  'next
	...
	#tag Method, Flags = &h21
		Private Sub drawBackground()
		  ...
		  //Rect background
		  me.graphics.foreColor = UsedBackground
		  me.graphics.FillRect (16,0,time2coordinate(demo.getDemoEndTime) - 16,me.height)
		  
		  me.graphics.foreColor = UnusedBackground
		  me.graphics.FillRect (time2coordinate(demo.getDemoEndTime),0,me.width - 16,me.height)
		  me.graphics.FillRect (16,0,time2coordinate(demo.getDemoStartTime) - 16,me.height)
		  
		  //Less zoom button
		  me.graphics.drawpicture lesszoom,0,0
	...
	#tag Method, Flags = &h21
		Private Sub drawBar(barID as string)
		  ...
		  me.graphics.ForeColor = EffectFillColor
		  me.graphics.FillRoundRect (startTime, layer, duration, elementHeight - 1, 5 ,5)
		  me.graphics.ForeColor = EffectBorderColor
		  me.graphics.DrawRoundRect (startTime, layer, duration, elementHeight - 1, 5, 5)
		  ...
		  if endTime - startTime > 16 then
		    ...
		    me.Graphics.ForeColor = EffectTextColor
		    me.graphics.DrawString (theString, startTime + 2, 11 + layer, duration - 4, true)
		  end if
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub drawBars()
		  ...
		  barIDs = demo.GetBarsToDraw(ruleStartTime, coordinate2time(me.Graphics.Width))
	...
	#tag Method, Flags = &h21
		Private Sub drawEndMarker()
		  ...
		  me.graphics.foreColor = LineColor
		  me.graphics.drawline (positionX,0,positionX,me.height)
		  me.graphics.drawpicture (markergreenright, positionX - 5, 5)
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub drawStartMarker()
		  ...
		  if positionX >= 16 then
		    me.graphics.foreColor = LineColor
		    me.graphics.drawline (positionX,0,positionX,me.height)
		    me.graphics.drawpicture (markergreenleft, positionX - 5, 5)
		  end if
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub drawTimeMarker()
		  ...
		  if demo.GetDataFolder.Name = "data" then
		    ...
		    LineColor = &c660000
		    positionX = time2coordinate(engines.EnginesList(engines.mainEngine).runTime)
		    
		    if positionX >= 16 then
		      me.graphics.foreColor = LineColor
		      me.graphics.drawline (positionX,0,positionX,me.height)
		      me.graphics.drawpicture (markerred, positionX - 5, 5)
		    end if
		    
		    SetCurrentTime engines.EnginesList(engines.mainEngine).runTime
		  end if
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub drawTimeRule()
		  ...
		  // TimeLine Background
		  me.graphics.foreColor = RuleBackground
		  me.graphics.FillRect (16,0,me.width - 16, 16)
		  
		  // TimeLine Rectangle
		  me.graphics.foreColor = RuleBorderColor
		  me.graphics.DrawRect (16,0,me.width - 16, 16)
		  ...
		  while x < me.width
		    me.graphics.foreColor = RuleDivisionsColor
		    me.graphics.DrawLine (x, 1, x, 5)
		    
		    me.graphics.drawstring (cstr(round(100 * coordinate2time(x)) / 100), x, 13)
		    
		    me.graphics.foreColor = RuleBackgroundLines
		    me.graphics.DrawLine (x, 16, x, me.height)
		    x = x + 25
		  wend
	...
	#tag Method, Flags = &h21
		Private Sub drawTrackRule()
		  ...
		  me.graphics.foreColor = RuleBackground
		  me.graphics.FillRect (0,16,16, me.height)
		  
		  // TrackLine Rectangle
		  me.graphics.foreColor = RuleBorderColor
		  me.graphics.drawRect (0,16,16, me.height)
		  
		  // Track separators
		  x = 16
		  while x < me.height
		    me.graphics.foreColor = RuleBackgroundLines
		    me.graphics.DrawLine (16, x, me.width, x)
		    x = x + elementHeight
		  wend
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub drawUsedTimes()
		  ...
		  // Used layer
		  me.graphics.drawpicture (markeryellowhorizontal, 3, 19 + elementHeight*layer)
	...
	#tag Method, Flags = &h1
		Protected Sub SetDemoEndTime(newTime as single)
		  ...
		  if oldTime <> demo.getDemoEndTime then engines.setEndTime demo.getDemoEndTime
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Sub SetDemoStartTime(NewTime as single)
		  ...
		  if oldTime <> demo.getDemoStartTime then engines.setStartTime demo.getDemoStartTime
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub SetSelectionEndTime(theTime as single)
		  ...
		  // engines.setBarEndTime (IDsList, theTime)
	...
	#tag Method, Flags = &h1
		Protected Sub SetSelectionSquare(x0 as double, y0 as double, x1 as double, y1 as double)
		  ...
		  //We draw the selection square
		  #pragma DisableBackgroundTasks
		  me.graphics.drawRect x0, y0, x1 - x0, y1 - y0
		End Sub
	#tag EndMethod
